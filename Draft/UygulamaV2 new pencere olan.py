...
from PyQt5.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QWidget, QPushButton, QTextEdit, QLabel, QHBoxLayout, QSizePolicy, QFileDialog
...
class ResultDialog(QDialog):
    def __init__(self, results):
        ...
        self.table.setHorizontalHeaderLabels(['İşlem No', 'Veri İsmi', 'Uygulanan Yöntem', 'Başarı Oranı'])
        self.table.setRowCount(len(results))

        for i, (data_name, method_name, success_rate) in enumerate(results, start=1):
            self.table.setItem(i - 1, 0, QTableWidgetItem(str(i)))
            self.table.setItem(i - 1, 1, QTableWidgetItem(data_name))
            self.table.setItem(i - 1, 2, QTableWidgetItem(method_name))
            self.table.setItem(i - 1, 3, QTableWidgetItem(f'{success_rate:.2f}%'))

        layout.addWidget(self.table)

        show_result_button = QPushButton('Görsel Olarak Görüntüle', self)
        show_result_button.clicked.connect(self.show_selected_result)
    ...
    def show_selected_result(self):
        ...
        if selected_row >= 0:
            # Burada seçilen sonucun grafiksel gösterimini yapabilirsiniz.
            pass
...
class PackingApp(QMainWindow):
    def __init__(self):
        ...
        self.rectangles = []
        self.bins = []
        self.results = []  # Yapılan işlemlerin sonuçlarını tutacak liste
        self.result_dialog = None  # Sonuçların gösterileceği pencere
        self.initUI()
    ...
    def initUI(self):
        ...
        self.btn_show_results = QPushButton('Sonuçları Göster', self)
        self.btn_show_results.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding)
        self.btn_show_results.setStyleSheet('font-size: 16px; background-color: #34495e; color: white;')
    ...
    def show_results(self):
        self.result_dialog = ResultDialog(self.results)
    ...
    def run_next_fit(self):
        ...
        self.results.append((data_name, 'Next-Fit', success_rate))
    ...
    def run_genetic_algorithm(self):
        ...
        success_rate = (total_fitted_area / total_rectangles_area) * 100
        self.results.append((data_name, 'Genetik Algoritma', success_rate))
